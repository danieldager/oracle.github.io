...
Full => models for which the inputs are segments of varying lengths
...
class SegmentDatasetFull(Dataset):
...
def get_sequences(folders, path, inputs=None):
...
def split_data(train_folders=None, test_folders=None, inputs=None):
    ...
    if train_folders or test_folders:
        ...
        train_data = get_sequences(train_paths, path, inputs)
        ...
        if test_folders:
            ...
            test_data = get_sequences(test_paths, path, inputs)
    ...
    # if no arguments are specified, split all trial data
        ...
        sequences = get_sequences(folder_names, path, inputs)
...
def load_data_full(train_data, eval_data, test_data, batch_size):
    train_dataset = SegmentDatasetFull(train_data)
    eval_dataset = SegmentDatasetFull(eval_data)
    test_dataset = TestDatasetFull(test_data)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_full)
    eval_loader = DataLoader(eval_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_full)
